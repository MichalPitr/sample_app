...
class PasswordResetForm < PasswordResets
  def setup
    ...
    # Not yet stored in database, so some divergence between @user and @reset_user
...
class PasswordResetFormTest < PasswordResetForm
  test 'reset with valid email' do
    # Not yet saved to DB, so @reset_user will have a digest while @user will have nil digest
  ...
  test 'reset with wrong email' do
    # equivalent to clicking the reset link, but replacing the email incorrectly
  ...
  test 'reset with correct email but invalid token' do
    # equivalent to clicking the reset link, but replacing the email incorrectly
    get edit_password_reset_path('invalid token', email: @reset_user.email)
    assert_redirected_to root_url
  end

  test 'reset with valid information' do
    # equivalent to clicking the reset link, but replacing the email incorrectly
    get edit_password_reset_path(@reset_user.reset_token, email: @reset_user.email)
    assert_template 'password_resets/edit'
    # validate that the form has @reset_user's email as a hidden field
